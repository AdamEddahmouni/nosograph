...
async def llm_extract(
    ...
    sources: str = Query(
        "pubmed,preprints,clinical_trials", min_length=1, description="Comma-separated source types"
    ),
    max_articles: int = Query(20, ge=1, le=100, description="Max articles to extract from"),
    model: str = Query(
