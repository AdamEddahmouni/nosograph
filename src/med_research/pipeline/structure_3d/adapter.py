"""Adapter around ``structure_3d.engine`` analysis."""

from __future__ import annotations

from pathlib import Path
from typing import Any

from typing_extensions import Unpack

from med_research.pipeline.adapter_options import AdapterOptions
from med_research.pipeline.base import BasePipelineModule
from med_research.pipeline.provenance import ProvenanceMetadata, build_provenance
from med_research.pipeline.registry import register_module
from med_research.pipeline.reporting import render_report
from med_research.pipeline.results import Structure3DResult


@register_module
class Structure3DModule(BasePipelineModule[Structure3DResult]):
    """Adapter for 3D structural target docking & AlphaFold evaluation."""

    _COVERAGE_MODULE = "structure_3d"

    @property
    def module_id(self) -> str:
        return "structure_3d"

    def coverage_inputs(self) -> tuple[str, ...]:
        return ("genes",)

    def run(self, disease_id: str, **opts: Unpack[AdapterOptions]) -> Structure3DResult:
        from med_research.pipeline.structure_3d.engine import analyze_structure_3d

        return analyze_structure_3d(
            disease_id=disease_id,
            progress_callback=opts.get("progress_callback"),
        )

    def report(
        self,
        results: Structure3DResult,
        disease_id: str,
        *,
        provenance: ProvenanceMetadata | None = None,
    ) -> Path:
        output_dir = Path("dist/reports")
        output_dir.mkdir(parents=True, exist_ok=True)
        report_path = output_dir / f"structure_3d_{disease_id}.html"

        prov = provenance or self.build_provenance(disease_id)

        html = render_report(
            template_name="reports/structure_3d.html",
            context={
                "disease_id": disease_id,
                "results": results,
                "provenance": prov,
            },
            disease_id=disease_id,
            provenance=prov,
        )
        report_path.write_text(html, encoding="utf-8")
        return report_path

    def build_provenance(
        self, disease_id: str, **opts: Unpack[AdapterOptions]
    ) -> ProvenanceMetadata:
        extra: dict[str, Any] = {
            key: value for key, value in opts.items() if key not in {"sources", "cache_or_live"}
        }
        return build_provenance(
            disease_id=disease_id,
            module=self.module_id,
            sources=["genes.json"],
            cache_or_live="cache",
            **extra,
        )
