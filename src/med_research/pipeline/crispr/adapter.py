"""Adapter around ``crispr.engine`` analysis."""
...
from med_research.pipeline.results import CrisprResult


@register_module
class CrisprModule(BasePipelineModule[CrisprResult]):
    """Adapter for CRISPR & gene therapy feasibility evaluation."""

    _COVERAGE_MODULE = "crispr"
    ...
    def run(self, disease_id: str, **opts: Unpack[AdapterOptions]) -> CrisprResult:
        from med_research.pipeline.crispr.engine import evaluate_crispr_feasibility

        return evaluate_crispr_feasibility(
    ...
    def report(
        ...
        *,
        ...
        report_path = output_dir / f"crispr_{disease_id}.html"

        prov = provenance or self.build_provenance(disease_id)

        html = render_report(
            template_name="reports/crispr.html",
            context={
                "disease_id": disease_id,
                "results": results,
                "provenance": prov,
