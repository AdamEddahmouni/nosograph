...
> **Mitigated 2026-08-07.** `RateLimitMiddleware` in `src/med_research/web/middleware.py` provides in-memory sliding-window rate limiting per client IP (`RATE_LIMIT_REQUESTS` / `RATE_LIMIT_WINDOW` env vars). Redis-backed or distributed limiting for multi-instance deployments **remains open as of 2026-08-08** — not addressed by the locking/verification work.
