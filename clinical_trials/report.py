...
def generate_ct_report(results: dict) -> str:
    """Generate an HTML report from clinical trial tracking results."""
...
<head>
    ...
    <title>Lupus Clinical Trial Tracker Report</title>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .stat-card:hover {{ border-color: #4b5563; transform: translateY(-2px); }}
        ...
        td {{ padding: 10px 14px; border-bottom: 1px solid #1a1a24; vertical-align: top; }}
        ...
        .match-card:hover {{ border-color: #4b5563; }}
        .match-header {{ display: flex; justify-content: space-between; margin-bottom: 6px; }}
        .match-title {{ font-weight: 600; font-size: 0.85rem; }}
        ...
        .hit-card h4 {{ font-size: 0.85rem; margin-bottom: 10px; font-weight: 600; }}
        .hit-bar {{
            display: flex; align-items: center; gap: 10px; margin-bottom: 6px;
        }}
        .hit-label {{ font-size: 0.75rem; font-weight: 500; min-width: 100px; }}
        .hit-fill {{
            height: 8px; border-radius: 4px; min-width: 3px;
            transition: width 0.3s;
        }}
        .hit-count {{ font-size: 0.72rem; color: #787890; min-width: 30px; }}
        ...
        @media (max-width: 768px) {{
            ...
            .gene-drug-grid {{ grid-template-columns: 1fr; }}
        }}
    </style>
</head>
<body>
    <div class="container">
        <div class="hero">
            <h1>Clinical Trial Tracker Report</h1>
            <p class="subtitle">
                Tracking {stats['total_trials']} Lupus Interventional Trials ·
                {stats['kg_matched_trials']} Cross-Referenced Against the Knowledge Graph
            </p>
            <p class="subtitle" style="font-size:0.78rem;margin-top:8px;">
                Data: ClinicalTrials.gov · Generated: {datetime.now().strftime('%B %d, %Y at %H:%M')}
            </p>
        </div>
        ...
        <div class="charts-row">
            {f'<div class="chart-card"><h4>Trials by Phase</h4><img src="data:image/png;base64,{phase_chart_b64}" alt="Phase Distribution Chart"/></div>' if phase_chart_b64 else '<div class="chart-card"><p class="subtitle" style="padding:40px">📊 Install matplotlib for phase charts</p></div>'}
            {f'<div class="chart-card"><h4>Mechanism of Action Categories</h4><img src="data:image/png;base64,{moa_chart_b64}" alt="MoA Chart"/></div>' if moa_chart_b64 else ''}
        </div>

        <h2 class="section-title">🔬 KG-Cross-Referenced Trials</h2>
        {matched_html}

        <h2 class="section-title">🧬💊 Top Genes & Drugs in Clinical Development</h2>
        {genes_drugs_html}

        <h2 class="section-title">📋 All Tracked Trials</h2>
        <div class="table-container" style="max-height:600px;overflow-y:auto;">
            {trials_table_html}
        </div>

        <footer>
            <p>Lupus Clinical Trial Tracker · Data from <a href="https://clinicaltrials.gov" target="_blank">ClinicalTrials.gov</a></p>
            <p>Cross-referenced against the <a href="../knowledge_graph/web/index.html">Lupus Knowledge Graph</a></p>
            <p style="margin-top:8px;color:#6b7280;">
                ⚠️ Disclaimer: Trial data reflects ClinicalTrials.gov records as of report generation.
                Verify trial status and results on the official registry.
            </p>
        </footer>
    </div>
</body>
