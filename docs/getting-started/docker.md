...
Local evaluation builds pass `DOCKER_SKIP_DISEASE_VALIDATE=1` so image build does not run strict validation across the 10k+ scaffold registry (that gate is expected to fail on scaffolds). This does **not** disable API keys or production `DEBUG=false` rules.
