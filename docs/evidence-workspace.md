...
|---|---|---:|---|
| `disease_id` | string | `sle` | Must be a discovered disease module (any discoverable `Disease.list_all()` id), such as `sle`, `ra`, `ms`, `ss`, `ssc`, `t1d`, `ibd`, `ad`, `als`, `as`, `asthma`, `atopic_dermatitis`, `copd`, `gout`, `pd`, `psa`, `pso`, or `t2d`. |
