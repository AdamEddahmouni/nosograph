...
The release metadata records 2,445 offline tests selected in the v0.2.1 release suite. That number is a labeled snapshot, not a live collect-gate. Suite growth since v0.2.1 is new unit modules (DEMO_MODE, a11y, harvest, license SBOM, public-metadata, CSP), on the order of +34 tests; passed vs collected can differ by skips, so CI does not assert an exact live integer.
...
| `make test-browser` | Playwright Chromium | Deterministic dashboard a11y, Evidence Workspace, and Evidence Explorer workflows. Install once with `python -m playwright install chromium`. |
